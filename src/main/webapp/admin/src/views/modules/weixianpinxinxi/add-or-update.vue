	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.weixianpinbianhao" label="危险品编号" prop="weixianpinbianhao">
					<el-input v-model="ruleForm.weixianpinbianhao" placeholder="危险品编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.zhuyishixiang" label="注意事项" prop="zhuyishixiang">
					<span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}'>{{ruleForm.zhuyishixiang}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'"  label="详情" prop="xiangqing">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.xiangqing" label="详情" prop="xiangqing">
                    <span :style='{"border":"1px solid #5497f2","padding":"0 40px","color":"#bbbbbb","borderRadius":"20px 40px","display":"inline-block","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.xiangqing"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			ruleForm: {
				weixianpinbianhao: this.getUUID(),
				weixianpinmingcheng: '',
				weixianpinleixing: '',
				guige: '',
				shuliang: '',
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='weixianpinleixing'){
							this.ruleForm.weixianpinleixing = obj[o];
							this.ro.weixianpinleixing = true;
							continue;
						}
						if(o=='guige'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssmlyz86/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weixianpinxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weixianpinxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
